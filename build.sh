...
#
# Usage:
#   ./build.sh              # release build + run tests (default)
#   ./build.sh dev          # portable release (HFT_NATIVE=OFF) + tests
#   ./build.sh debug        # ASan/UBSan debug build + tests
#   ./build.sh bench        # release build with benchmarks, then list them
#   ./build.sh perf         # Linux-only: native+LTO release for real perf runs
...
case "$MODE" in
  release)
    configure build -DCMAKE_BUILD_TYPE=Release -DHFT_BUILD_TOOLS=ON
    cmake --build build -j
    ctest --test-dir build --output-on-failure
    ;;
  dev)
    # Portable: won't tune for this CPU. Matches CI. Numbers here are NOT perf.
    configure build -DCMAKE_BUILD_TYPE=Release -DHFT_NATIVE=OFF -DHFT_BUILD_TOOLS=ON
    cmake --build build -j
    ctest --test-dir build --output-on-failure
    ;;
  debug)
    configure build-debug -DCMAKE_BUILD_TYPE=Debug -DHFT_NATIVE=OFF
    cmake --build build-debug -j
    ctest --test-dir build-debug --output-on-failure
    ;;
  bench)
    configure build -DCMAKE_BUILD_TYPE=Release -DHFT_BUILD_BENCH=ON
    cmake --build build -j
    echo "--- benchmarks built (run on Linux, pinned) ---"
    find build -maxdepth 1 -type f -name 'bench_*' -perm -u+x
    ;;
  perf)
    if [ "$OS" != "Linux" ]; then
    ...
    ;;
  clean)
    rm -rf build build-debug build-perf build-asan
